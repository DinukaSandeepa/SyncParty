...
export default function RoomSection({ room, setRoom, isInRoom, onJoinRoom, onLeaveRoom }) {
    return (
        <div className="p-6 mb-6 transition-all duration-300 delay-100 border bg-white/5 rounded-xl border-white/5 animate-fade-in-up hover:border-white/20">
            ...
            <div className="flex gap-3">
                ...
                {isInRoom ? (
                    <button
                        onClick={onLeaveRoom}
                        className="px-6 py-2 font-medium text-white transition-all duration-300 bg-red-600 rounded-lg hover:bg-red-700 hover:scale-105 active:scale-95"
                    >
                        Leave Room
                    </button>
                ) : (
